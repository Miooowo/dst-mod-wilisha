...
AddReplicableComponent('event_trigger_role_wilisha')

AddPlayerPostInit(function (inst)
    inst:ListenForEvent('event_trigger_role_wilisha_triggered',function ()
        if inst == ThePlayer and inst.replica.event_trigger_role_wilisha then
            local event_name = inst.replica.event_trigger_role_wilisha:GetEventName() or ''
            if event_name == '' then
                return
            end
            local type = inst.replica.event_trigger_role_wilisha:GetType() or ''
            if type == '' then
                return
            end
    ...
    inst:AddComponent('event_trigger_role_wilisha')
end)
...
function EntityScript:PushEvent(event,data,...)
    ...
    if allevents[event] then
        -- 筛选player
        if self and self.components.event_trigger_role_wilisha then
            local type -- 被哪个type拦了
            -- hook
            local allow = true
            local equips = SUGAR_role_wilisha:getAllEquipments(self)
            ...
            if not allow then
                ...
                if type then
                    -- 传递数据
                    self.components.event_trigger_role_wilisha:Push(event,type)
                end
                return
            end
        end
...
function State:HandleEvent(sg,eventame,data,...)
    ...
    if allevents[sg] then
        if inst and inst.components.event_trigger_role_wilisha then
            local allow = true
            local equips = SUGAR_role_wilisha:getAllEquipments(inst)
